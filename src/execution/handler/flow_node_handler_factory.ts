...
export class FlowNodeHandlerFactory implements IFlowNodeHandlerFactory {
    ...
    public async create<TFlowNode extends Model.Base.FlowNode>(flowNode: TFlowNode, processModelFascade: IProcessModelFascade): Promise<IFlowNodeHandler<TFlowNode>> {
    ...
    private async _decorateWithBoundaryEventHandlers<TFlowNode extends Model.Base.FlowNode>(boundaryEvents: Array<Model.Events.BoundaryEvent>, processModelFascade: IProcessModelFascade, handlerToDecorate: IFlowNodeHandler<Model.Base.FlowNode>): Promise<IFlowNodeHandler<TFlowNode>> {
    ...
    private async _createBoundaryEventHandler(boundaryEventNode: Model.Events.BoundaryEvent, handlerToDecorate: IFlowNodeHandler<Model.Base.FlowNode>): Promise<IFlowNodeHandler<Model.Base.FlowNode>> {
    ...
    private _orderBoundaryEventsByPriority(boundaryEvents: Array<Model.Events.BoundaryEvent>): void {
        ...
        // TODO: (SM) order the boundary events so that e.g. the error handler only handles the error of the actual flow node,
